...
layout(set = 0, binding = 0, std430) readonly buffer XeTextureBlitScratchSource {
  uvec2 xe_texture_blit_scratch_source[];
};

layout(set = 0, binding = 1, rgba16ui) uniform writeonly uimage2DArray
...
void main() {
  ...
  uvec2 value = xe_texture_blit_scratch_source[
  ...
  imageStore(xe_texture_blit_scratch_dest,
             ivec3(int(block_index.x),
                   ...
                   int(block_index.z)),
             // rg32ui would need StorageImageExtendedFormats, which the device
             // does not enable, so the block is written as four 16-bit
             // components - the same eight bytes in the same order.
             uvec4(value.x & 0xFFFFu, value.x >> 16u, value.y & 0xFFFFu,
                   value.y >> 16u));
}
